...
// To alloc a tensor
wtensor wekuaTensorEmpty(wekuaContext ctx, uint32_t ndim, uint64_t *shape, uint8_t com, uint8_t dtype);

// To get matrix with random elements
// wtensor wekuaTensorRandn(wekuaContext ctx, uint32_t ndim, uint64_t *shape, uint8_t com);

// To get matrix with random numbers in the range [a, b) or [a, b] depending on rounding.
// wtensor wekuaTensorRandUniform(wekuaContext ctx, uint32_t ndim, uint64_t *shape, void *ra, void *ia, void *re, void *ie, uint8_t dtype);
...
// 	wekuaContext ctx, uint32_t ndim, uint64_t *shape,
// 	void *buffer, uint8_t dtype, uint8_t com
// );

// To reshape a tensor
// void wekuaTensorReshape(
// 	wtensor tensor, uint32_t ndim, uint64_t *shape,
