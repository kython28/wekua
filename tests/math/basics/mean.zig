...
fn test_mean(
    ...
    if (is_complex) {
        ...
        try wekua.math.basic.mean(T, command_queue, tensor, &actual_result, &actual_result_complex);

        var expected_result: T = 0;
        var expected_result_complex: T = 0;

        const number_of_elements = tensor.dimensions.number_of_elements_without_padding / 2;
        for (0..number_of_elements) |i| {
            const index = i * 2;
        ...
        expected_result /= @floatFromInt(number_of_elements);
        expected_result_complex /= @floatFromInt(number_of_elements);
        ...
        try wekua.math.basic.mean(T, command_queue, tensor, &actual_result, null);

        var expected_result: T = 0;
        for (buffer1) |v| {
            expected_result += v;
        }
        const number_of_elements = tensor.dimensions.number_of_elements_without_padding;
        expected_result /= @floatFromInt(number_of_elements);
    ...
    inline for (&.{f32, f64}) |T| {
        inline for (&.{true, false}) |is_complex| {
            inline for (&.{true, false}) |vector_enabled| {
                try test_mean(T, ctx, command_queue, is_complex, vector_enabled);
            }
        }
    }
}
