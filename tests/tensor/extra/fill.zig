...
test "fill and check" {
    ...
    const tensor = try wekua.tensor.alloc(ctx, &[_]u64{20, 10}, .{.dtype = wekua.tensor.dtypes.wTensorDtype.uint64});
    ...
    wekua.tensor.event.register_new_event(w_cmd, tensor, null, null, custom_event, .read, true) catch |err| {
        // tensor.mutex.unlock();
        _ = std.c.pthread_mutex_unlock(tensor.mutex);
        return err;
    };
