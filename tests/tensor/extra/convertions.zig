...
fn test_convertions_to_real(
    ...
    try wekua.tensor.extra.convertions.to_real(w_cmd, tensor, tensor2, dom);
...
test "Convert complex tensor to real" {
    ...
    const randprg = std.crypto.random;
    inline for (types) |T| {
        try test_convertions_to_real(allocator, ctx, randprg, T, false);
        try test_convertions_to_real(allocator, ctx, randprg, T, true);
    }
}
