...
const UserCallback = Set.UserCallback;
...
pub fn init(
    ...
    const buf_len = @sizeOf(Combined) + @sizeOf(cl.event.Event) * total_events;
    const buf = try allocator.alloc(u8, buf_len);
    errdefer allocator.free(buf);

    const set: *Combined = @alignCast(@ptrCast(buf.ptr));
    const prev_events: ?[]cl.event.Event = blk: {
        if (total_events == 0) break :blk null;

        const array = @as(
            [*]cl.event.Event,
            @alignCast(@ptrCast(buf.ptr + @sizeOf(Combined))),
        )[0..total_events];
...
pub inline fn getPrevEvents(self: *const Combined) ?[]const cl.event.Event {
