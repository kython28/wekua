    ...
    return struct {
        ...
        fn executeAdagrad(
            ...
            if (x.flags.is_complex) {
                try set_arg(kernel, 6, @sizeOf(T), @ptrCast(&self.config.lri));
            }
        ...
        fn step(
            ...
            for (cache.slots) |*slot| {
                ...
                for (weights, gradients) |w, g| {
                    try self.executeAdagrad(command_queue, w, g, gradient_histories[index]);
                    _index += 1;
                }
                ...
                if (layer.getBiasGradients(slot.cache)) |bias_gradients| {
                    ...
                    for (bias.?, bias_gradients) |b, maybe_bg| {
                        ...
                        try self.executeAdagrad(command_queue, b.?, bg, bias_gradient_histories[index]);
                    }
                }
